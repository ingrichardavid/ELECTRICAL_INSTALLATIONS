...
public class ControllerArea implements ActionListener, KeyListener, WindowListener, ChangeListener {
    ...
    private TypeRush typeCaliberIluminaria, typeCaliberPowerPoint;
    private double breakdownVoltage;
    private double potencyTotalOld;
        ...
        if (validate_fields()) {
            if (ServiceArea.update(new Area(
                    ...
                    new Project(
                            ...
                            0, 
                            null), 
                    (viewArea.getArea().getPotency_total() - potencyTotalOld) + MethodsForCalculationsIluminariaPowerPoint.potencyInIluminariaAndPowerPoint(
                            MethodsForCalculationsIluminariaPowerPoint.potencyInIluminaria(Double.valueOf(viewArea.getJspArea().getValue().toString())),
                            MethodsForCalculationsIluminariaPowerPoint.potencyInPowerPoint(Integer.valueOf(viewArea.getTxtQuantityPowerPoint().getValue().toString()))),                     
                    (viewArea.getArea().getNeutral() - potencyTotalOld) + MethodsForCalculationsIluminariaPowerPoint.potencyInIluminariaAndPowerPoint(
                            MethodsForCalculationsIluminariaPowerPoint.potencyInIluminaria(Double.valueOf(viewArea.getJspArea().getValue().toString())),
                            MethodsForCalculationsIluminariaPowerPoint.potencyInPowerPoint(Integer.valueOf(viewArea.getTxtQuantityPowerPoint().getValue().toString()))), 
                    1),fill_area_iluminaria_powerPoint())){            
        ...
        switch (typeOfBranchCircuitInArea){                
            case ILUMINARIA:                    
                if (!viewArea.getrBtnAirIluminaria().isSelected() && !viewArea.getrBtnGroundIluminaria().isSelected()){
                    ...
                    viewArea.getrBtnAirIluminaria().requestFocus();
                } else {
            ...
            case POWER_POINT:      
                if (!viewArea.getrBtnAirPowerPoint().isSelected() && !viewArea.getrBtnGroundPowerPoint().isSelected()){
                    MessagesStructure.Warning(MessagesStructure.format(200, messages.getProperty(Messages.RUSH_NO_FOUND), MessagesStructure.justify));
                    viewArea.getrBtnAirPowerPoint().requestFocus();
                } else {
    ...
    /**
     * Método para calcular la Potencia Total acumulada anteriormente, para luego restar a la potencia nueva.
     * @return  Retorna la Potencia Total
     */
    private void potencyTotalOld(){
        if (viewArea.getTitle().equalsIgnoreCase("Modificar Área")){
            potencyTotalOld = MethodsForCalculationsIluminariaPowerPoint.potencyInIluminariaAndPowerPoint(
                            MethodsForCalculationsIluminariaPowerPoint.potencyInIluminaria(Double.valueOf(viewArea.getJspArea().getValue().toString())),
                            MethodsForCalculationsIluminariaPowerPoint.potencyInPowerPoint(Integer.valueOf(viewArea.getTxtQuantityPowerPoint().getValue().toString())));
        }
    }//Fin del Método
